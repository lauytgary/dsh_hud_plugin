...
`;
		 ...
		 * composer card (`[data-composer-card]`, the centered ~748px input card)
		 * — the dock slot itself renders inside the full-width composer bar, so
		 * measuring the slot's parent would read the whole column width and
		 * always report ~0 space. Falls back to "full" whenever the anchor
		 * cannot be found or anything throws, so the HUD can never disappear
		 * because of a measurement failure.
		 */
		function useTier(rootRef) {
			...
			react.useLayoutEffect(() => {
				...
				const measure = () => {
					...
					const anchor = el?.closest('[data-composer-card]')
				...
				try {
					...
					if (typeof ResizeObserver !== "undefined") {
						...
						const anchor = rootRef.current?.closest('[data-composer-card]')
							?? rootRef.current?.closest('[data-slot="conversation.composer.dock"]')?.parentElement
							?? null;
