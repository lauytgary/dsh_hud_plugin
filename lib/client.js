...
`;
		...
		//#endregion
		//#region instruments
		/** Local 24h clock with DeepSeek peak/off-peak rate badge (Beijing 09-12 / 14-18 = PEAK). */
		function LocalClock() {
			...
			return react.createElement("div", { className: `gsh-clock${peak ? " gsh-peak" : " gsh-off"}` },
				...
				react.createElement("span", { className: "gsh-clock-rate" }, peak ? L.peak : L.offPeak));
		}
		...
		//#endregion
		//#region panel
		/** Responsive tier from the free space right of the chat column (px). */
		function tierOf(space) {
			if (space >= 200) return "full";
			if (space >= 90) return "mini";
			return "hidden";
		}
		/**
		 * Measure the free space right of the chat column (the anchor is the
		 * composer dock panel — our own root stays display:none in the hidden
		 * tier, so the anchor's rect is always real). Re-measures on viewport
		 * resize and on any layout change of the anchor (sidebar drag, details
		 * drawer, chat width).
		 */
		function useTier(rootRef) {
			const [tier, setTier] = react.useState(() => {
				if (typeof window === "undefined") return "full";
				return tierOf((window.innerWidth ?? 1500) * 0.42);
			});
			react.useLayoutEffect(() => {
				const measure = () => {
					const el = rootRef.current;
					const slot = el === null ? null : el.closest('[data-slot="conversation.composer.dock"]');
					const anchor = slot?.parentElement ?? el?.parentElement ?? null;
					if (anchor === null) return;
					const rect = anchor.getBoundingClientRect();
					setTier(tierOf(Math.max(0, window.innerWidth - rect.right)));
				};
				measure();
				const ro = new ResizeObserver(measure);
				const anchor = rootRef.current?.closest('[data-slot="conversation.composer.dock"]')?.parentElement;
				if (anchor !== void 0 && anchor !== null) ro.observe(anchor);
		...
		const GameStatsPanel = react.memo(function GameStatsPanel({ useSession, useProjection }) {
			...
			return react.createElement("div", {
				...
				react.createElement(LocalClock, null),
				mini
