...
@media (prefers-reduced-motion: reduce){.gsh-root.gsh-running,.gsh-running .gsh-fill{animation:none}.gsh-fill,.gsh-ctx-segs,.gsh-needle,.gsh-strip{transition:none}}
`;
		 ...
		 * always reads the proportion between the two. With no recorded time
		 * (llmMs = toolMs = 0) the track stays empty.
		 */
		function ChannelBar({ llmMs, toolMs }) {
			...
			return react.createElement("div", { className: "gsh-inst" },
				...
				react.createElement("div", { className: "gsh-track" },
					total > 0 && react.createElement("div", { className: "gsh-dual" },
		...
		function SpeedGauge({ tps }) {
			const redline = Math.max(200, Math.ceil(tps / 100) * 100);
			const pct = tps / redline;
			const angle = pct * 180;
			...
			return react.createElement("div", { className: "gsh-gauge" },
				...
				react.createElement("svg", { role: "img", "aria-label": `${L.throughput} ${tps > 0 ? formatTps(tps) : "—"} ${L.tokPerS}`, viewBox: "0 0 100 62", width: "118", height: "68", style: { display: "block", margin: "-2px auto 0" } },
				...
				react.createElement("div", { className: "gsh-gauge-read" },
					react.createElement("span", { className: "gsh-value", style: { fontSize: 13, color: "#e6f2ff" } }, tps > 0 ? formatTps(tps) : "—"),
		...
		function useTier() {
			...
			react.useLayoutEffect(() => {
				...
				try {
					...
					if (typeof ResizeObserver !== "undefined") {
						...
						if (anchor !== null) roRef.current.observe(anchor);
			...
			/**
			 * Callback ref: the layout effect above runs on the component's first
			 * commit, when the panel may still be hidden (stats not available →
			 * rootRef.current is null). Re-measure and re-bind the ResizeObserver
			 * to the composer anchor the moment the panel actually mounts, so the
			 * tier is correct without waiting for a window resize.
			 */
			const setRoot = react.useCallback((el) => {
				rootRef.current = el;
				if (el === null || measureRef.current === null) return;
				roRef.current?.disconnect();
				const anchor = findComposerCard(el) ?? el.closest('[data-slot="conversation.composer.dock"]')?.parentElement ?? null;
				if (anchor !== null) roRef.current?.observe(anchor);
				roRef.current?.observe(document.body);
				measureRef.current();
			}, []);
			return [setRoot, tier, space];
		}
		...
		const GameStatsPanel = react.memo(function GameStatsPanel({ useSession, useProjection }) {
			...
			const [setRoot, tier, space] = useTier();
		...
		//#endregion
		//#region test-only surface
		if (typeof process !== "undefined" && process.env?.DSH_HUD_TEST === "1") {
			exports.__test = { formatTokens, formatDuration, formatTps, tierOf, billedInputTokens, cacheHitPercent };
		}
		//#endregion
		exports.apply = apply;
