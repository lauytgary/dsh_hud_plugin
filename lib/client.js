...
`;
		...
		function tierOf(space, width) {
			...
			return space >= 200 ? "full" : "mini";
		}
		/**
		 * Measure the free space right of the chat column (anchor = the composer
		 * dock panel). Falls back to "full" whenever the anchor cannot be found
		 * or anything throws, so the HUD can never disappear because of a
		 * measurement failure.
		 */
