...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = `
.gsh-root{--gsh-cyan:#4dd7ff;--gsh-violet:#c084fc;--gsh-blue:#5b8cff;--gsh-green:#4ade80;--gsh-amber:#ffb454;--gsh-red:#ff6b6b;--gsh-magenta:#f472b6;box-sizing:border-box;position:fixed;right:12px;top:50%;transform:translateY(-50%);z-index:900;pointer-events:none;width:164px;display:flex;flex-direction:column;gap:9px;padding:10px 10px 11px;border:1px solid rgba(100,170,255,.28);border-radius:12px;background:linear-gradient(180deg,rgba(14,22,44,.92),rgba(7,11,26,.94));box-shadow:0 0 14px rgba(70,140,255,.14),inset 0 0 22px rgba(70,140,255,.05);font-family:var(--dsw-font-family,Inter,system-ui,sans-serif)}
...
@media (max-width:1150px){.gsh-root{display:none}}
...
`;
		...
		const GameStatsPanel = react.memo(function GameStatsPanel({ useSession, useProjection }) {
			...
			return react.createElement("div", { className: `gsh-root${running ? " gsh-running" : ""}` },
